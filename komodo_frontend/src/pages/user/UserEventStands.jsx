  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) setError(e.response?.data?.detail || 'Failed to load stands')
      } finally {
  ...
  return (
    <div className="purchase-page">
      <header className="purchase-header">
        ...
        <h1 className="purchase-title text-glow-primary">{event?.name ?? 'Stands'}</h1>
        <p className="purchase-subtitle">Choose a stand to see products</p>
      </header>
      {error && (
      ...
      <div className="purchase-grid">
        {stands.length === 0 && !error && (
          <p className="purchase-empty">No stands available for this event.</p>
        )}
        {stands.map((stand) => (
          <Link
            key={stand.id}
